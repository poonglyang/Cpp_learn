...
public:
	...
	/// 장비류가 들어가는 인벤토리 백터
	/// </summary>
	std::vector<ItemBase*> equipable;

	/// <summary>
	/// 소모품류가 들어가는 인벤토리 백터
	/// </summary>
	std::vector<ItemBase*> consumables;
	...
	/*std::vector<ItemBase*> GetInventory(InventoryNum num) {
		switch (num)
		{
	...
	template<typename T>
	int AddItemAndReturnRemaining(int id, int count, std::vector<T> inventory);
