...
std::vector<RoomNode*> RoomGenerator::GenerateRoomInGivenSpaces(std::vector<Node*> roomSpaces, float roomBottomCornerModifier, float roomTopCornerModifier, int roomOffset)
{
    ...
    {
        myMath::Vector2Int newBottomLeftPoint = StructureHelper::GenerateBottomLeftCornerBetween(
            space->GetBottomLeftCorner(), space->GetTopRightCorner(),
            roomBottomCornerModifier, roomOffset);     // 방의 새로운 좌하단 코너 좌표를 반환

        myMath::Vector2Int newTopRightPoint = StructureHelper::GenerateTopRightCornerBetween(
            space->GetBottomLeftCorner(), space->GetTopRightCorner(),
        ...
        space->SetBottomRightCorner(myMath::Vector2Int(newTopRightPoint.x, newBottomLeftPoint.y));
        space->SetTopLeftCorner(myMath::Vector2Int(newBottomLeftPoint.x, newTopRightPoint.y));
